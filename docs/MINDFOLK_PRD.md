...
| [CHANGED] **Therapist Discovery** | Browse therapists with filtering and matching | P0 | Grid/list view with filters; 5-category tag system; Matching algorithm implementation; Favorites functionality; No video profiles [TODO] |
| **Chemistry Call Booking** [TODO] | Multi-session booking with calendar integration | P0 | 15-min free calls + 30/60-min paid options; Native calendar picker with month navigation; Session type selection, recurring options; Additional comments field; Payment method selection; Appointment confirmation with security assurances |
| [CHANGED] **Direct Booking** | Book sessions without chemistry calls | P0 | Calendar integration UI; Session type selection (30/60/90 min); Appointment creation in database; No payment processing [TODO] |
| **Session Management** | Complete appointment lifecycle | P0 | Dashboard with Today/Upcoming/Past sections; "In X days (/hours/minutes)" countdown that switches to a "JOIN NOW" button 5 minutes before appointment time [TODO]; Session reminders with tiering notifications [TODO]; Live video interface with chat panel [TODO]; Post-call feedback collection after chemistry call (confidential) [TODO]; Post-session testimonial option (after verification) [TODO] |
...
| **Favorites & Communication** | Enhanced therapist management | P0 | Dedicated favorites screen with search; Upcoming appointments indicator [TODO]; "BOOK NOW" button within search ("Book Free Chemistry Call" AND "Book Therapy Session" if the user has not previously had a chemistry call that has taken place with that therapist previous, otherwise just "Book Therapy Session") [TODO]; Notifications system (forms, summaries, homework) [TODO]; File attachments and sharing [TODO] |
| **Payment Processing** [TODO] | Pay-per-session with transparent pricing | P0 | Stripe integration for client to pay the rate set by the therapist, with 15% of that going to Mindfolk as a commission and 85% to therapist; Multiple payment methods; Automated receipts; Refund/dispute handling; Session-based billing |
| **Account Management** | Profile and preferences | P0 | Email verification, secure authentication; Notification settings [TODO]; Document preferences [TODO]; Data privacy management; Photo upload [TODO]; Help centre and FAQs [TODO], customer support ticketing [TODO] |
...
| Section | Line # | Feature/Item | Current Status | What's Left to Do | Priority | Recommended Implementation | Potential Risks | Schema Coverage | RLS Status | DB Optimization | Implementation % | Migration Path | Sprint Number |
|---------|--------|--------------|----------------|-------------------|----------|----------------------------|------------------|-----------------|------------|-----------------|------------------|----------------|
| Product Vision | 15 | Therapist intro videos | Not implemented | Add video recording/upload for therapist profiles | P1 | Use browser MediaRecorder API for recording. Upload to Supabase Storage with size limits. Add video preview component similar to existing session room UI. Store video URL in therapist_profiles table which already has the field defined | Reduced differentiation | ✅ video_url col exists | N/A | No index on video_url | 10% | Add Storage bucket, Edge Function | Sprint 1.2 |
| Client Features | 485 | Video profiles | UI exists, no video | Integrate Cloudflare Stream for video hosting | P1 | Use for hosting therapist intro videos. Configure Cloudflare Stream API. Upload videos from the existing upload UI to Cloudflare. Store video URLs in therapist_profiles table. Add video player component with streaming optimization | Less engaging discovery | ✅ video_url col exists | N/A | No index on video_url | 10% | Cloudflare integration | Sprint 1.2 |
| Client Features | 486 | Chemistry Call Booking | Not implemented | Extend appointment system for 15-min free calls | P0 | Extend the existing appointment booking system to add a chemistry call type with 15-minute duration limit. Use the current calendar UI and appointment creation flow, but add logic to make these calls free and track conversion to paid sessions. The database structure already supports different appointment types | Missing key conversion feature | ✅ appointments table | ✅ Full RLS | ✅ Unique constraint | 30% | Add session_type enum value | Sprint 3.2 |
| Client Features | 487 | Direct Booking Payment | UI only | Integrate Stripe payment processing | P0 | The payment form UI components already exist at `/src/components/payment/`. Integrate Stripe using Supabase Edge Functions for secure server-side processing. Use the existing session_earnings table to track therapist payouts and platform fees. Add webhook handlers for payment events | Cannot transact | ✅ session_earnings | ✅ Therapist view only | ✅ Auto-calc trigger | 20% | Stripe webhook handlers | Sprint 3.1 |
| Client Features | 488 | Session countdown timer | Not implemented | Add React countdown component | P1 | The appointments page and database structure exist. Add a countdown timer component using React hooks. Switch to JOIN NOW button 5 minutes before appointment time | Poor user experience | ✅ appointments table | ✅ Full RLS | ✅ Indexed dates | 0% | Frontend only | Sprint 5.1 |
| Client Features | 488 | Session reminders | Not implemented | Setup Resend/Twilio notifications | P0 | Implement reminder notifications via Supabase Edge Functions with Resend/Twilio | Missed appointments | ✅ appointments table | ✅ Full RLS | Need cron job | 0% | pg_cron + Edge Function | Sprint 2.1 & 2.2 |
| Client Features | 488 | Live video interface | UI only | Integrate Daily.co WebRTC | P0 | The session room UI is ready for Daily.co integration. Integrate Daily.co using the existing session room UI at `/src/pages/session/SessionRoom.tsx`. Create Supabase Edge Functions to generate secure room tokens. Use the existing appointments table to trigger session creation and track attendance | Cannot deliver therapy | ✅ appointments table | ✅ Full RLS | ✅ Status tracking | 5% | Daily.co API integration | Sprint 4.1 |
| Client Features | 488 | Post-call feedback | Not implemented | Create feedback forms and tables | P1 | For post-call feedback, create a feedback form component that appears after session end time. Store feedback in a new `session_feedback` table | No quality metrics | ❌ No feedback table | N/A | N/A | 0% | Create feedback table | Post-MVP |
| Client Features | 488 | Post-session testimonial | Not implemented | Add review system | P2 | Add review component after session verification. Store in reviews table | No social proof | ✅ client_testimonials | ✅ Full RLS | ✅ Public view policy | 10% | Frontend component | Post-MVP |
| Client Features | 492 | Upcoming appointments | Partial | Join appointments with favorites | P2 | The favorites functionality is partially implemented. Enhance the existing favorites page to show upcoming appointments by joining with appointments table | Incomplete UX | ✅ Both tables exist | ❌ Favorites no RLS | Need join optimization | 40% | Add RLS to favorites | Sprint 1.1 |
| Client Features | 492 | Chemistry call logic | Not implemented | Add conditional booking buttons | P1 | Add conditional booking buttons based on past chemistry call history | Confusing flow | ✅ appointments table | ✅ Full RLS | Need session_type enum | 10% | Frontend logic only | Sprint 3.2 |
| Client Features | 492 | Notifications system | Not implemented | Create notifications table and UI | P1 | For notifications, create a `notifications` table and use Supabase Realtime for push updates | Poor engagement | ❌ No table | N/A | N/A | 0% | Create notifications table | Sprint 1.1 |
| Client Features | 492 | File attachments | Not implemented | Setup Supabase Storage with RLS | P2 | File attachments can use Supabase Storage with proper RLS policies | Cannot share documents | ❌ No storage bucket | N/A | N/A | 0% | Create storage bucket | Post-MVP |
| Client Features | 493 | Payment Processing | UI only | Full Stripe integration | P0 | The payment form UI components already exist at `/src/components/payment/`. Integrate Stripe using Supabase Edge Functions for secure server-side processing. Use the existing session_earnings table to track therapist payouts and platform fees. Add webhook handlers for payment events | No revenue | ✅ session_earnings | ✅ Full RLS | ✅ Auto-calc trigger | 20% | Stripe Edge Functions | Sprint 3.1 |
| Client Features | 494 | Notification settings | Not implemented | Add preferences UI | P2 | Email verification is handled by Supabase Auth. Extend the settings page to include notification preferences (stored in profiles table) | No user control | ✅ profiles table | ✅ Full RLS | Add JSONB column | 5% | Add preferences column | Post-MVP |
| Client Features | 494 | Document preferences | Not implemented | Add document management | P2 | Add document upload using Supabase Storage | GDPR compliance gap | ❌ No storage setup | N/A | N/A | 0% | Storage bucket + RLS | Post-MVP |
| Client Features | 494 | Photo upload | Not implemented | Add avatar upload | P2 | Add avatar upload to Supabase Storage with image optimization | Incomplete profiles | ✅ avatar_url column | ✅ Full RLS | Need storage bucket | 10% | Storage bucket setup | Post-MVP |
| Client Features | 494 | Help centre/FAQs | Not implemented | Create help pages | P1 | Create a help center page with FAQ content | Poor support | ❌ No FAQ table | N/A | N/A | 0% | Static content or CMS | Sprint 5.1 |
| Client Features | 494 | Support ticketing | Table exists, no UI | Build ticket submission UI | P1 | The support ticketing system table structure exists (`support_tickets`) but needs UI implementation | No support system | ✅ support_tickets | ❌ No RLS policies | Need status index | 30% | Add RLS policies | Sprint 1.1 & 5.1 |
| Therapist Features | 508 | Personalized greeting | Not implemented | Add dynamic welcome message | P2 | The dashboard page with widgets exists. Add personalized greeting using profile data | Less engaging | ✅ profiles data | ✅ Full RLS | Indexed | 0% | Frontend only | Post-MVP |
| Therapist Features | 508 | Appointment countdown | Not implemented | Reuse client countdown component | P2 | Create countdown timer component similar to client side | Inconsistent UX | ✅ appointments table | ✅ Full RLS | ✅ Date indexed | 0% | Frontend component | Sprint 5.1 |
| Therapist Features | 508 | Client status dashboard | Not implemented | Create client status tracking | P2 | Use existing appointments table to show upcoming sessions | Poor client management | ✅ appointments table | ✅ Full RLS | Need client_id index | 20% | Add aggregation query | Post-MVP |
| Therapist Features | 508 | Income tracking | Mock data only | Connect to real earnings data | P1 | Add income calculations using session_earnings table | No financial visibility | ✅ session_earnings | ✅ Therapist view | ✅ Auto-calc trigger | 40% | Connect UI to data | Sprint 1.2 |
| Therapist Features | 509 | Google/Outlook sync | Not implemented | OAuth2 calendar integration | P1 | The calendar page exists. Add OAuth2 integration for Google Calendar API and Microsoft Graph API for Outlook. Store tokens in encrypted format in profiles table. Use existing availability system as base | Manual scheduling | ✅ availability table | ✅ Full RLS | Need token storage | 10% | OAuth2 Edge Functions | Sprint 5.1 |
| Therapist Features | 509 | Calendar color-coding | Not implemented | Add appointment type colors | P2 | Add color-coding by mapping appointment types to colors | Poor visual organization | ✅ appointments table | ✅ Full RLS | Has session_type | 0% | Frontend styling | Post-MVP |
| Therapist Features | 509 | Session reminders | Not implemented | Notification system | P0 | Research and implement shadcn/ui calendar components | Missed sessions | ✅ appointments table | ✅ Full RLS | Need cron trigger | 0% | pg_cron + Edge Function | Sprint 2.3 |
| Therapist Features | 510 | Client status management | Not implemented | Add status field to relationships | P2 | The clients page exists. Extend with status field in a new client_relationships table | Poor organization | ❌ No relationships table | N/A | N/A | 0% | Create relationships table | Post-MVP |
| Therapist Features | 510 | Match percentage | Not implemented | Calculate reverse matching | P2 | Calculate match percentage using existing matching algorithm in reverse | Missing insights | ✅ Assessment data | ✅ Full RLS | ✅ Array indexes | 0% | Frontend calculation | Post-MVP |
| Therapist Features | 510 | Session note templates | Not implemented | Create template system | P1 | Create client file pages with tabs for different sections. Store session notes in existing client_session_notes table with template support | Inefficient documentation | ✅ client_session_notes | ✅ Full RLS | JSONB for templates | 20% | Add template field | Sprint 5.1 |
| Therapist Features | 510 | Progress tracking | Not implemented | Add metrics visualization | P2 | Add metrics visualization using existing charting library | No outcome tracking | ✅ client_session_notes | ✅ Full RLS | Has mood_rating | 0% | Frontend charts | Post-MVP |
| Therapist Features | 511 | GAD-7/PHQ-9 scales | Not implemented | Add assessment forms | P1 | Notes UI exists. Add standardized assessment forms (GAD-7, PHQ-9) as React components. Store scores in JSONB field in client_session_notes | No clinical tools | ✅ client_session_notes | ✅ Full RLS | JSONB field ready | 5% | Add form components | Sprint 5.1 |
| Therapist Features | 511 | Progress visualization | Not implemented | Add charts for metrics | P2 | Use existing charting library for progress visualization | No visual insights | ✅ session_notes data | ✅ Full RLS | Has metrics fields | 0% | Connect chart library | Post-MVP |
| Therapist Features | 511 | Communication workflow | Not implemented | Create workflow system | P2 | Add template system for common note types | Poor communication | ✅ client_session_notes | ✅ Full RLS | JSONB flexible | 0% | Add template system | Post-MVP |
| Therapist Features | 512 | Analytics - all metrics | Real data connected | Connect to real data sources | P1 | ✅ Connected useTherapistAnalytics hook to real database tables. Added functions for calculating earnings and appointments metrics from actual data. Time filtering for analytics data now working properly | Business metrics available | ✅ therapist_analytics | ✅ Full RLS | ✅ Auto-update trigger | 100% | ✅ Completed | Sprint 1.2 |
| Admin Features | 534 | Audit trail | Implemented | Add audit logging table | P1 | ✅ Created audit_trail table with RLS. Implemented automatic logging triggers for profiles, therapist_profiles, and appointments tables. Still need to add impersonation logging and UI | Basic compliance covered | ✅ audit_trail table | ✅ Full RLS | ✅ Triggers active | 70% | Add impersonation logging | Sprint 1.1 |
| Admin Features | 535 | Role change actions | Not implemented | Add suspension/deletion | P1 | Users and therapists pages exist. Add role change actions using Supabase Auth admin API. Implement soft delete with status field | Cannot manage users | ✅ profiles table | ✅ Admin full access | Has role enum | 10% | Auth API integration | Sprint 5.1 |
| Admin Features | 535 | Profile timeline | Not implemented | Add auth event tracking | P2 | Add auth event tracking using Supabase hooks | No user history | ❌ No event table | N/A | N/A | 0% | Create events table | Post-MVP |
| Admin Features | 535 | GDPR tools | Implemented | Add export/deletion | P1 | ✅ Created and deployed gdpr-export and gdpr-delete Edge Functions. Platform is now GDPR compliant with data export and deletion capabilities. Still need to improve UI for making requests | Legal compliance achieved | ✅ All user tables | ✅ Admin access | ✅ Functions deployed | 80% | Improve request UI | Sprint 1.1 |
| Admin Features | 535 | Bulk operations | Not implemented | Add checkbox selection UI | P2 | Create bulk operations UI with checkbox selection pattern | Inefficient management | ✅ All tables | ✅ Admin full access | UI only | 0% | Frontend batch UI | Post-MVP |
| Admin Features | 536 | Document previews | Not implemented | Add Storage preview | P1 | Basic approval workflow exists. Add document preview using Supabase Storage URLs | Cannot verify therapists | ✅ documents JSONB | ✅ Admin view | Need storage viewer | 10% | Storage integration | Sprint 1.1 |
| Admin Features | 536 | Mandatory reasons | Implemented | Add reason field | P2 | ✅ Added decision_reason textarea to TherapistApplications component. Approval/rejection now requires providing a reason | Decision tracking active | ✅ decision_reason col | ✅ Admin update | ✅ UI implemented | 100% | ✅ Completed | Sprint 1.1 |
| Admin Features | 536 | Request more info | Not implemented | Add request workflow | P2 | Create notification templates in Edge Functions | Incomplete verification | ✅ status enum | ✅ Admin update | Has under_review | 20% | Edge Function notify | Sprint 2.1 |
| Admin Features | 536 | Template notifications | Not implemented | Create email templates | P2 | Create notification templates in Edge Functions | Manual communication | ❌ No template table | N/A | N/A | 0% | Email templates | Sprint 2.1 |
| Admin Features | 536 | Decision audit trail | Not implemented | Add audit logging | P1 | Implement audit trail for all verification decisions | No accountability | ✅ reviewed_by cols | ✅ Admin update | Has timestamps | 30% | Add trigger logging | Sprint 1.1 |
| Admin Features | 537 | Content scanning | Implemented | Add keyword detection | P2 | ✅ Created moderation_queue table with RLS. Deployed content-moderation Edge Function with keyword detection for violence, self-harm, inappropriate content, and spam | Basic safety protection | ✅ moderation_queue | ✅ Full RLS | ✅ Keyword system live | 80% | Improve false positive handling | Sprint 1.1 |
| Admin Features | 537 | Appeal process | Not implemented | Create appeal workflow | P2 | Implement appeal process with status tracking. Add policy reference documentation page | No dispute resolution | ❌ No appeals table | N/A | N/A | 0% | Create appeals workflow | Post-MVP |
| Admin Features | 537 | Moderation inbox | Partially implemented | Implement queue system | P2 | ✅ Created moderation_queue table structure with priority, status tracking, and assignment fields. UI exists but needs to be connected to real data | Basic moderation ready | ✅ moderation_queue | ✅ Full RLS | ✅ Status tracking | 40% | Connect UI to queue | Sprint 5.1 |
| Admin Features | 537 | Media preview | Not implemented | Add preview components | P2 | Add media preview panels for content review | Cannot review content | ❌ No media storage | N/A | N/A | 0% | Storage integration | Post-MVP |
| Admin Features | 537 | Policy reference | Not implemented | Create policy docs | P2 | Add policy reference documentation page | No guidelines | ❌ No policy table | N/A | N/A | 0% | Static or CMS content | Post-MVP |
| Admin Features | 537 | Remove/escalate | Not implemented | Add action handlers | P2 | Add remove/escalate action handlers | Cannot moderate | ❌ No moderation table | N/A | N/A | 0% | Create action handlers | Post-MVP |
| Admin Features | 538 | All platform analytics | Mock data only | Connect real data | P1 | Overview page with KPI cards exists. Connect to real data sources: uptime from health checks, active sessions from auth, user growth from profiles table, revenue from session_earnings. Add time-series charts using existing charting library | No platform insights | ✅ All source tables | ✅ Admin view | Need aggregation | 20% | Connect real queries | Sprint 5.1 |
| Platform Core | 554 | Video Sessions | UI only | Daily.co integration | P0 | Integrate Daily.co using the existing session room UI at `/src/pages/session/SessionRoom.tsx`. Create Supabase Edge Functions to generate secure room tokens. Use the existing appointments table to trigger session creation and track attendance. The UI components are already in place with mock functionality | Cannot deliver therapy | ✅ appointments table | ✅ Full RLS | Need meeting_url col | 5% | Daily.co SDK + Edge | Sprint 4.1 |
| Platform Core | 555 | Payment Infrastructure | UI only | Stripe Connect setup | P0 | Integrate Stripe using the existing payment form UI components at `/src/components/payment/`. Create Supabase Edge Functions for server-side payment processing. Use the existing session_earnings table structure to track therapist earnings and platform fees. The database schema already supports financial tracking | No revenue | ✅ session_earnings | ✅ Full RLS | ✅ Auto-calc ready | 20% | Stripe API integration | Sprint 3.1 |
| Integrations | 600 | Daily.co | Not integrated | WebRTC implementation | P0 | The session room UI components are ready. Install @daily-co/daily-js SDK. Create Edge Function to generate room tokens with proper expiry. Update SessionRoom component to initialize Daily client. Add device selection UI using existing form components | No video calls | ✅ appointments table | ✅ Full RLS | UI components ready | 5% | NPM install + config | Sprint 4.1 |
| Integrations | 601 | Cloudflare Stream | Integrated | Video hosting setup | P1 | ✅ Integration complete. Edge Function created for video uploads. VideoUpload and VideoPlayer components implemented. Connected to therapist_profiles.video_url column for storage | Video profiles available | ✅ video_url column | N/A | No index on URL | 100% | ✅ Completed | Sprint 1.2 |
| Integrations | 602 | Stripe Connect | UI only | Payment processing | P0 | Payment form UI exists. Setup Stripe Connect onboarding for therapists. Create Edge Functions for payment processing. Use webhooks to update session_earnings table. Implement weekly payout scheduling. Add invoice generation for tax compliance | No transactions | ✅ session_earnings | ✅ Full RLS | ✅ Payout tracking | 20% | Stripe Connect OAuth | Sprint 3.1 |
| Integrations | 605 | PostHog | Not integrated | Analytics tracking | P1 | Install PostHog JS SDK. Configure privacy-compliant tracking. Create custom events for key actions (booking, matching, conversion). Build dashboards for product metrics. Export data to existing analytics pages | No product metrics | ✅ therapist_analytics | ✅ Full RLS | Ready for events | 0% | SDK install + config | Post-MVP |
| Integrations | 606 | Resend | Not integrated | Email notifications | P0 | Create Edge Functions for email sending. Design email templates matching brand. Setup triggers for key events (booking confirmation, reminders). Add unsubscribe management. Track email metrics | No communications | ✅ appointments data | ✅ Full RLS | Need email log table | 0% | Edge Functions + API | Sprint 2.1 |
| Integrations | 607 | Twilio | Not integrated | SMS notifications | P0 | Setup Twilio account with UK phone numbers. Create Edge Functions for SMS sending. Implement appointment reminders. Add opt-in/opt-out management. Handle international number formatting | No reminders | ✅ appointments table | ✅ Full RLS | Need pg_cron | 0% | Edge Function + cron | Sprint 2.2 |