-- Migration: fix_function_search_paths_final
-- Author: MindFolk Foundation Agent
-- Date: 2025-09-28
-- Purpose: Fix security vulnerability - add explicit search_path to existing functions only
-- Issue: 9 functions flagged by Supabase security advisor for mutable search_path
-- Risk: Low - security hardening, no functional changes
-- Note: Only fixing functions that actually exist and are being flagged

-- Fix 1: calculate_profile_strength function
-- This function calculates how complete a therapist's profile is
...
-- Fix 2: update_profile_strength_trigger function
-- This automatically updates profile strength when therapist data changes
...
-- Fix 3: generate_personality_tags function
-- This creates personality tags for matching therapists with clients
...
-- Fix 4: audit_trigger_function function
-- This logs all changes to important tables for compliance using audit_trail table
...
BEGIN
    ...
    -- Initialize the audit record with basic info
...
-- Fix 5: update_onboarding_progress function
-- This tracks therapist progress through the onboarding system
...
-- Fix 6: update_updated_at_column function
-- This automatically updates the updated_at timestamp on table changes
...
-- Note: Removed log_impersonation_event and log_impersonation_end functions
-- because they reference non-existent 'impersonation_events' table
-- Impersonation logging appears to be handled through audit_trail table instead

-- Verification query - confirm functions now have explicit search_path
SELECT
    proname as "Function Name",
    prosrc LIKE '%search_path%' as "Has Search Path Set"
FROM pg_proc
WHERE proname IN (
