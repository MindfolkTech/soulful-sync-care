  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        head_row: "flex",
        head_cell:
          "text-[--text-muted] rounded-md w-[--touch-target-comfort] font-normal text-[0.8rem]",
        row: "flex w-full mt-[--space-xs]",
        cell: "h-[--touch-target-comfort] w-full text-center text-sm p-0 relative [&:has([aria-selected])]:bg-[--surface-accent] first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20",
        day: cn(
          buttonVariants({ variant: "ghost" }),
          "h-[--touch-target-comfort] w-[--touch-target-comfort] font-normal aria-selected:opacity-100"
        ),
        day_selected:
          "bg-[--garden-green] text-[--text-on-brand] hover:bg-[--garden-green] hover:text-[--text-on-brand] focus:bg-[--garden-green] focus:text-[--text-on-brand]",
        day_today: "bg-[--surface-accent] text-[--text-primary]",
        day_outside:
          "text-[--text-muted] opacity-50 aria-selected:bg-[--surface-accent]/50 aria-selected:text-[--text-muted] aria-selected:opacity-30",
        day_disabled: "text-[--text-muted] opacity-50",
        day_range_middle:
          "aria-selected:bg-[--surface-accent] aria-selected:text-[--text-primary]",
