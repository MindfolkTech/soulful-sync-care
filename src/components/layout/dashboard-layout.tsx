...
export function DashboardLayout({ children, title, subtitle }: DashboardLayoutProps) {
  ...
  return (
    <div className="grid grid-rows-[auto_1fr] min-h-dvh overflow-hidden bg-background">
      {/* Header Section */}
      <header className="row-start-1 bg-jovial-jade text-white px-6 py-4 relative z-50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className={`relative ${sidebarOpen ? 'hidden md:block' : 'block'}`}>
      ...
      <div className="row-start-2 flex overflow-hidden">
        ...
        <aside className={`
          ...
          w-64 bg-surface-accent min-h-full py-6
          transform transition-transform duration-300 ease-in-out
          ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
        `}>
          <nav className="space-y-2 px-4 h-full flex flex-col">
            {/* Main navigation */}
            ...
            <div className="border-t border-border pt-4">
              <button className="flex items-center gap-3 px-3 py-2 text-jovial-jade hover:text-garden-green transition-colors min-h-touch-min">
        ...
        <main className="flex-1 overflow-auto min-w-0">
          <Container className="py-8 min-h-full">
            {/* Page title section */}
            {(title || subtitle) && (
              <div className="mb-8">
                {title && (
